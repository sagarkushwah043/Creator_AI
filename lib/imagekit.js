...
export const buildTransformationUrl = (src, transformations = []) => {
  ...
  const transformParams = transformations
    ...
    // 👇 FIXED: use comma instead of colon
    .join(",");

  try {
    if (!src) return src;

    // 👇 FIXED: correct /tr:/ pattern
    const transformationPath = `tr:${transformParams}/`;

    if (src.includes("/tr:")) {
      // Already has transformations, replace existing
      return src.replace(/\/tr:[^/]*\//, `/${transformationPath}`);
    }

    // Insert before filename
    const urlParts = src.split("/");
    const fileIndex = urlParts.length - 1;
    urlParts.splice(fileIndex, 0, transformationPath);
    return urlParts.join("/");
  } catch (err) {
    console.error("Error building transformation URL:", err);
    return src;
  }
};
